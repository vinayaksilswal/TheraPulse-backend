...
Sender: support@lumively.com (configured via settings)

Prerequisites:
  1. Resend account with API key (set RESEND_API_KEY in env)
  2. DNS domain verification for lumively.com (MX, SPF, DKIM records)
...
async def send_email_blast(
    ...
    *,
    ...
    try:
        ...
        for u in users:
            email_set.add(u.email.lower().strip())
