...
import { getAccessToken, getCJProducts, getCachedToken, disconnectCJ, getLogs, queryCJProduct, addImportedProduct, getImportedProducts, getStorefrontProducts, updateStorefrontProduct, deleteStorefrontProduct, resetStorefrontProducts, bulkAiRewriteProducts, moveStorefrontProduct } from '../services/cjApi';
...
function DashboardContent({ onLogout }) {
  ...
  useEffect(() => {
    ...
    const fetchSales = async () => {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        throw new Error('API server returned an HTML error page. Node.js backend is not running.');
      }
      const data = await res.json();
        if (data.success) {
          setSalesRecords(data.list);
        } else {
          setSalesRecords([]);
        }
      } catch (e) {
  ...
  const handleGenerateLinkSubmit = async (e) => {
    e.preventDefault();
    alert("Manual CJ order creation from the frontend has been disabled for security reasons.");
  };
