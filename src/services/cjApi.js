...
export const getAccessToken = async (force = false) => {
  ...
  try {
    const response = await fetchWithRetry(
      '/api-cj/api2.0/v1/authentication/getAccessToken',
      {
        method: 'POST',
...
/**
 * Fetches storefront products list with deterministic sorting
 */
export const getCJProducts = async (page = 1, size = 50) => {
  ...
  logger.info(`Loaded catalog: ${list.length} products total.`);
...
// Idempotency guard: track recently created order numbers
const recentOrderNumbers = new Set();

/**
 * Creates CJ Dropshipping order with idempotency protection
 */
export const createCJOrder = async (products, recipient) => {
  const orderNumber = recipient.orderId || `TP-${Date.now()}`;

  // Idempotency check
  if (recentOrderNumbers.has(orderNumber)) {
    logger.warn(`Duplicate order creation blocked: ${orderNumber}`);
    return { success: true, orderId: orderNumber, mode: 'Deduplicated' };
  }
  recentOrderNumbers.add(orderNumber);
  // Cleanup old entries after 60 seconds
  setTimeout(() => recentOrderNumbers.delete(orderNumber), 60000);

  const mappedCjItems = products.map((item) => {
    if (item.pid === '1798542129166426112') {
      const originalVid = item.variant?.vid || item.vid || 'mask-standard';
      let mappedVid = originalVid;

      if (originalVid.startsWith('mask-')) {
        mappedVid = originalVid === 'mask-premium' ? 'cj-variant-set3' : 'cj-variant-set1';
      }

      return { ...item, pid: '1798542129166426112', vid: mappedVid, originalVid };
    }
    return item;
  });

  const cjItems = mappedCjItems;

  if (cjItems.length === 0) {
    logger.info('Order contains only local products. Bypassing CJ API order creation.');
    return { success: true, orderId: orderNumber, mode: 'Local-Fulfillment' };
  }

  let activeToken = getCachedToken();
  if (!activeToken) {
    logger.info('No token found. Authenticating before order creation...');
  ...
  for (let i = 0; i < cjItems.length; i++) {
    const item = cjItems[i];
    let vid = item.vid;

    // Resolve local mask variants to live CJ variant IDs
    if (item.originalVid && item.originalVid.startsWith('mask-')) {
      try {
        await cjRateLimiter.waitForToken();
        const response = await fetchWithRetry(
          '/api-cj/api2.0/v1/product/variant/query?pid=1798542129166426112',
          {
            method: 'GET',
            headers: { 'CJ-Access-Token': activeToken, 'Content-Type': 'application/json' },
          },
          { service: 'cj', operation: 'resolveVariant' }
        );
        if (response.ok) {
          const result = await response.json();
          if (result.code === 200 && result.data?.length > 0) {
            if (item.originalVid === 'mask-premium') {
              const found =
                result.data.find((v) => v.variantKey?.toLowerCase().includes('set3')) ||
                result.data[1] ||
                result.data[0];
              vid = found.vid;
            } else {
              const found =
                result.data.find((v) => v.variantKey?.toLowerCase().includes('set')) ||
                result.data[0];
              vid = found.vid;
            }
            logger.info(`Resolved variant "${item.originalVid}" → CJ VID "${vid}"`);
          }
        }
      } catch (err) {
        logger.warn(`Variant resolution failed, using fallback`, { error: err.message });
      }
    }
  ...
  const orderPayload = {
    orderNumber,
    shippingCustomerName: recipient.fullName || recipient.name || 'Customer',
    shippingAddress: recipient.address || '',
    shippingAddress2: '',
    shippingCity: recipient.city || '',
    shippingProvince: recipient.state || '',
    shippingZip: recipient.zip || '',
    shippingCountryCode: recipient.countryCode || 'US',
    shippingCountry: recipient.country || 'United States',
  ...
  logger.info(`Creating order: ${orderNumber}`, {
    itemCount: formattedProducts.length,
    recipient: { city: orderPayload.shippingCity, country: orderPayload.shippingCountry },
  });

  try {
    await cjRateLimiter.waitForToken();
    const response = await fetchWithRetry(
      '/api-cj/api2.0/v1/shopping/order/createOrderV2',
      {
        method: 'POST',
        headers: { 'CJ-Access-Token': activeToken, 'Content-Type': 'application/json' },
        body: JSON.stringify(orderPayload),
      },
      { service: 'cj', operation: 'createOrder' }
    );
    ...
    if (result.code === 200 && result.data) {
      logger.info(`Order ${orderNumber} created successfully via CJ API!`, { mode: 'Live' });
      return { success: true, orderId: result.data.orderId || orderNumber, mode: 'Live' };
    } else {
      throw new Error(result.message || 'Order creation failed');
    }
  } catch (err) {
    logger.warn(`Order creation API failed: "${err.message}". Using fallback simulation.`, {
      orderId: orderNumber,
      code: err.code,
    });
    ...
    return { success: true, orderId: orderNumber, mode: 'Fallback-Simulation' };
  }
};
...
export const queryCJProduct = async (pid) => {
  ...
  if (typeof pid === 'string' && pid.toUpperCase().startsWith('CJ')) {
    ...
    try {
      ...
      const varRes = await fetchWithRetry(
        `/api-cj/api2.0/v1/product/variant/query?productSku=${pid}`,
        {
          method: 'GET',
          headers: { 'CJ-Access-Token': activeToken, 'Content-Type': 'application/json' },
        },
      ...
      if (resolvedPid === pid) {
        ...
        const searchRes = await fetchWithRetry(
          `/api-cj/api2.0/v1/product/listV2?page=1&size=5&keyWord=${pid}`,
          {
            method: 'GET',
            headers: { 'CJ-Access-Token': activeToken, 'Content-Type': 'application/json' },
          },
  ...
  try {
    await cjRateLimiter.waitForToken();
    const response = await fetchWithRetry(
      `/api-cj/api2.0/v1/product/query?pid=${resolvedPid}`,
      {
        method: 'GET',
        headers: { 'CJ-Access-Token': activeToken, 'Content-Type': 'application/json' },
      },
...
export const getProductVariants = async (pid) => {
  ...
  try {
    await cjRateLimiter.waitForToken();
    const response = await fetchWithRetry(
      `/api-cj/api2.0/v1/product/variant/query?pid=${pid}`,
      {
        method: 'GET',
        headers: { 'CJ-Access-Token': activeToken, 'Content-Type': 'application/json' },
      },
