...
import { createCJOrder } from '../services/cjApi';
...
export default function CheckoutDrawer({ isOpen, onClose, cart, clearCart }) {
  ...
  const handlePaypalCheckoutSuccess = useCallback(async (details) => {
    ...
    try {
      ...
      if (capturedAmount) {
        ...
        if (!amountCheck.valid) {
          ...
          throw new Error(`Payment amount verification failed: ${amountCheck.error}`);
        }
      }

      // Extract shipping data from PayPal with null-safety
      const shipping = details.purchase_units?.[0]?.shipping;
      const customerData = {
        email: formData.email || details.payer?.email_address || '',
        fullName: formData.fullName || shipping?.name?.full_name || 'Guest',
        address: formData.address || shipping?.address?.address_line_1 || '',
        city: formData.city || shipping?.address?.admin_area_2 || '',
        state: formData.state || shipping?.address?.admin_area_1 || '',
        zip: formData.zip || shipping?.address?.postal_code || '',
        phone: formData.phone || '',
        countryCode: shipping?.address?.country_code || 'US',
        gateway: 'PayPal',
      };

      // Flag missing phone for manual follow-up
      if (!customerData.phone || customerData.phone === '0000000000') {
        logger.warn('PayPal order missing phone number — flag for manual follow-up', {
          orderId: details.id,
        });
      }

      // Create CJ fulfillment order
      const cjRes = await createCJOrder(cart, customerData);

      if (!cjRes.success) {
        throw new Error(cjRes.error || 'Failed to create fulfillment order');
      }

      const orderId = details.id || `TP-PP-${Math.floor(Math.random() * 900000) + 100000}`;
      recordSale({
        customer: customerData,
        items: cart,
        cjOrderId: cjRes.orderId,
        orderId,
        total,
      });
                        ...
                        <PayPalButtons
                              ...
                              purchase_units: [
                                {
                                  ...
                                  description: 'TheraPulse Purchase',
                                },
                              ],
                            });
                          }}
                          onApprove={async (data, actions) => {
                            try {
                              ...
                              await handlePaypalCheckoutSuccess(details);
                            } catch (err) {
